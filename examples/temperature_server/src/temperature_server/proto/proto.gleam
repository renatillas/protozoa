...
/// HTTP GET /v1/sensors/{sensor_id}/temperatures
...
pub fn http_create_temperature(
  ...
  case req.method {
    http.Post -> {
      case decode.run(req.body, with: create_temperature_request_decoder()) {
        Ok(proto_request) -> {
          case handler(proto_request) {
            Ok(response) -> {
              response.new(200)
        ...
        Error(_) -> {
          response.new(400)
          |> response.set_body(
            mist.Bytes(bytes_tree.from_string("Invalid request body")),
          )
        }
      }
    }
...
/// HTTP PUT /v1/sensors/{sensor_id}/temperatures
/// Accepts a handler function that processes the request
pub fn http_update_temperature(
  ...
  case req.method {
    http.Put -> {
      case decode.run(req.body, with: update_temperature_request_decoder()) {
        Ok(proto_request) -> {
          case handler(proto_request) {
            Ok(response) -> {
              response.new(200)
...
/// HTTP DELETE /v1/locations/{location}/sensors/{sensor_id}
/// Accepts a handler function that processes the request
...
/// HTTP PATCH /v1/temperatures/search
/// Accepts a handler function that processes the request
pub fn http_search_temperatures(
  ...
  case req.method {
    http.Patch -> {
      case decode.run(req.body, with: search_temperatures_request_decoder()) {
        Ok(proto_request) -> {
          case handler(proto_request) {
            Ok(response) -> {
              response.new(200)
...
fn format_error_response(
  ...
  case error {
    ...
    InvalidRequest(msg) ->
      ...
      |> response.prepend_header("content-type", "text/plain")
      |> response.set_body(
        mist.Bytes(bytes_tree.from_string("Invalid Request: " <> msg)),
      )
    InternalError(msg) ->
      response.new(500)
      |> response.prepend_header("content-type", "text/plain")
