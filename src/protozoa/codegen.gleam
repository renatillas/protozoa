...
import gleam/result
import gleam/string
import protozoa/codegen/decoders
import protozoa/codegen/encoders
import protozoa/codegen/types
import protozoa/internals/type_registry.{type TypeRegistry}
import protozoa/parser.{type ProtoFile, type Path}
import simplifile
...
fn generate_file_with_imports(
  ...
  let message_types = types.generate_types_with_registry(proto_file.messages, registry, file_path)
  let message_encoders = encoders.generate_encoders_with_registry(proto_file.messages, registry, file_path)
  let message_decoders = decoders.generate_decoders_with_registry(proto_file.messages, registry, file_path)
  let enum_helpers = encoders.generate_enum_helpers_with_nested(proto_file.enums, proto_file.messages)
...
fn generate_file_header(proto_file_path: String) -> String {
  "//// Generated by Protozoa from " <> proto_file_path <> "\n" <>
  "//// \n" <>
...
fn determine_needed_imports(proto_file: ProtoFile) -> List(String) {
  ...
  let needs_list = has_repeated_fields(proto_file) || has_field_mask_reference(proto_file)
  ...
  let needs_option = has_oneofs(proto_file)
...
fn get_field_well_known_types(message: parser.Message) -> List(String) {
  let field_types = message.fields
  ...
  let oneof_types = message.oneofs
