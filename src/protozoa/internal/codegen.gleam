...
////
//// This module is purely functional and does not perform file I/O. It takes parsed ProtoFiles
//// and returns generated code as strings.
...
import protozoa/parser/file.{type Path, type ProtoFile}
import protozoa/parser/proto

/// Generate code for multiple proto files combined into a single output.
...
fn has_enums(proto_file: ProtoFile) -> Bool {
  ...
  || proto_file.messages
  |> list.any(fn(msg) { !list.is_empty(msg.nested_enums) })
}
...
fn has_enum_fields(proto_file: ProtoFile) -> Bool {
  ...
  |> list.any(fn(msg) {
    ...
    |> list.any(fn(field) {
      case field.field_type {
        proto.EnumType(_) -> True
        proto.Repeated(proto.EnumType(_)) -> True
        proto.Optional(proto.EnumType(_)) -> True
        _ -> False
      }
    })
  })
...
fn has_bytes_fields(proto_file: ProtoFile) -> Bool {
  ...
  |> list.any(fn(msg) {
    ...
    |> list.any(fn(field) {
      case field.field_type {
        proto.Bytes -> True
        proto.Repeated(proto.Bytes) -> True
        proto.Optional(proto.Bytes) -> True
        _ -> False
      }
    })
  })
...
fn has_message_fields(proto_file: ProtoFile) -> Bool {
  ...
  |> list.any(fn(msg) {
    ...
    |> list.any(fn(field) {
      case field.field_type {
        proto.MessageType(_) -> True
        proto.Repeated(proto.MessageType(_)) -> True
        proto.Optional(proto.MessageType(_)) -> True
        _ -> False
      }
    })
  ...
  |> list.any(fn(service) {
    ...
    |> list.filter(fn(method) {
      case method.http_method {
        option.Some(proto.Get) | option.Some(proto.Delete) -> True
        _ -> False
      }
    })
    |> list.any(fn(method) {
      ...
      |> result.map(fn(msg) {
        ...
        |> list.any(fn(field) {
          case field.field_type {
            proto.Float | proto.Double -> True
            proto.Repeated(proto.Float) | proto.Repeated(proto.Double) -> True
            proto.Optional(proto.Float) | proto.Optional(proto.Double) -> True
            _ -> False
          }
        })
      })
...
fn get_field_well_known_types(message: proto.Message) -> List(String) {
  let field_types =
    message.fields
    |> list.fold([], fn(acc, field) {
      case field.field_type {
        proto.MessageType(name) ->
          case well_known_type.is_well_known_type(name) {
            True -> [name, ..acc]
            False -> acc
          }
        proto.Repeated(proto.MessageType(name)) ->
          case well_known_type.is_well_known_type(name) {
            True -> [name, ..acc]
            False -> acc
          }
        proto.Optional(proto.MessageType(name)) ->
          case well_known_type.is_well_known_type(name) {
            True -> [name, ..acc]
            False -> acc
          }
...
fn generate_service_stubs(services: List(proto.Service)) -> String {
  case services {
    [] -> ""
    _ -> {
      services
...
fn generate_single_service_stub(service: proto.Service) -> String {
