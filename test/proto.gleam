...
pub fn get_temperature_service(
  ...
  case decode.run(request_bytes, with: temperature_request_decoder()) {
    ...
    Error(_) -> Error(DecodeError("Failed to decode TemperatureRequest"))
  }
}
...
pub fn delete_temperature_service(
  ...
  case decode.run(request_bytes, with: delete_request_decoder()) {
    ...
    Error(_) -> Error(DecodeError("Failed to decode DeleteRequest"))
  }
}
...
fn handler_error_to_http_response(error: TemperatureServiceError) -> response.Response(BitArray) {
  case error {
    NotFound ->
      response.Response(
        status: 404,
